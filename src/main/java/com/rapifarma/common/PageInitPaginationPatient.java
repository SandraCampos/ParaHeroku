...
@Component
public class PageInitPaginationPatient {
	@Autowired
	...
	public  ModelAndView initPagination(Optional<Integer> pageSize, Optional<Integer> page, 
		...
		Page<Patient> patientList = this.patientService.findAll(PageRequest.of(evalPage, evalPageSize));
		PagerModel pager = new PagerModel(patientList.getTotalPages(), patientList.getNumber(), BUTTONS_TO_SHOW);

		initModelView.addObject("patientsList", patientList);
	...
	public  ModelAndView initPaginationByName(Optional<Integer> pageSize, Optional<Integer> page, 
		...
		Page<Patient> patientListByName = this.patientService.fetchByName(name,PageRequest.of(evalPage, evalPageSize));
		PagerModel pager = new PagerModel(patientListByName.getTotalPages(), patientListByName.getNumber(), BUTTONS_TO_SHOW);

		initModelView.addObject("patientsList", patientListByName);
		initModelView.addObject("selectedPageSize", evalPageSize);
		initModelView.addObject("pageSizes", PAGE_SIZES);
		initModelView.addObject("pager", pager);
